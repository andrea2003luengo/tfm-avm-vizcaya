...
st.set_page_config(
    page_title="AVM Vizcaya - Stacking híbrido", 
    layout="centered",
...
st.write("Introduzca las características del inmueble para promediar su precio de mercado a través de la arquitectura de Stacking híbrido.")
st.markdown("---")
...
medianas_photos = {
    ...
    "Urbi": 25.0, "Berango": 30.0, "Ensanche-Moyua": 23.0, "Zabalburu-Diputación": 26.0,
    ...
    "Kabiezes": 25.0, "La Magdalena": 25.0, "La Txitxarra - Murrieta - Parke Santurtzi": 25.0,
    "Larrea - San Juan de Dios - Peñota": 25.0, "Las Viñas": 25.0, "Mamariga": 25.0, "Villar - San Juan": 25.0,
    "Zona Centro": 25.0, "Asilo - Rebonza - Urbinaga": 25.0, "Centro - Albiz - Markonzaga": 25.0,
    "La Paz - El Carmen - Anunciación": 25.0, "La Unión - Vista Alegre": 25.0, "Sopelana": 25.0,
    "Valle de Trapaga-Trapagaran": 25.0, "Zalla": 25.0
}

# --- MEDIANAS DE DISTANCIA AL CENTRO POR BARRIO (METROS) ---
medianas_distance = {
...
with col1:
    ...
    size = st.number_input("Superficie (m²)", min_value=30, max_value=600, value=85, step=5)
...
if botón_tasar:
    with st.spinner("Procesando simulación a través de los ecosistemas del Stacking..."):
        try:
            # 1. Construcción del DataFrame en crudo (Exactamente con las 27 columnas originales esperadas)
            raw_entry = pd.DataFrame([{
                ...
                'status': str(status),
                'highlight': 'Estandar',
                'numPhotos': int(medianas_photos.get(neighborhood, 25.0)),
                'showAddress': True,
                'distance': float(medianas_distance.get(neighborhood, 5500.0)),
                'hasVideo': False,
                'hasPlan': False,
                'has3DTour': False,
                'has360': False,
